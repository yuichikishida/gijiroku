...
├── 用語集.txt                     ← 人名・社名・専門用語(1行1語、#はコメント)。
│                                    Whisperのinitial_promptと議事録生成の表記統一に使われる
...
- `index.html` — 録音Webアプリ(単一ファイル、依存なし)
  ...
  - 議事録のその場生成: Gemini API(gemini-2.5-flash、無料枠あり)。キーはlocalStorageに保存。
    生成物は必ずファイルとして残す(Drive保存、未設定・失敗時はWord形式をダウンロード)。
    保存先未設定時はボタンが赤い警告表示になる
  - Driveの用語集.txtは保存先フォルダ経由で読み込み、議事録生成プロンプトに注入される
