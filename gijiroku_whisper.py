...
出力:
  ...
  議事録/文字起こし/〈元ファイル名〉_議事録.md            ← Gemini APIで自動生成(キー設定時のみ)
...
def transcribe_file(audio_path: str, out_dir: str, done_dir: str) -> None:
    ...
    try:
        ...
        minutes = generate_minutes(build_prompt(name, date, text), api_key)
        minutes_path = os.path.join(out_dir, f"{name}_議事録.md")
